...
| Theme ID Эксид+Экслантикс | уточнить через `Theme.fetchThemesByAccount` |
...
| 15.05.2026 | Добавлена документация BA Feed API (`api.br-analytics.ru`): авторизация login+sig(MD5), эндпоинт сообщений, методология тональности. Theme ID Эксид+Экслантикс в Feed API уточнить. Автоматизация `fetch_messages.js` пока настроена только для ba-linni. |
