...
│   │       ├── 2025-12.json
│   │       └── ...
│   ├── tonal/                  ← ручные экспорты сообщений из BA (JSON/CSV)
│   │   ├── parse.js            ← конвертер экспорта → messages-YYYY-MM.json
│   │   └── *.json / *.csv      ← сырые выгрузки
...
| Theme ID Линнимакс | `14159496` (отличается от Statistical API) |
...
| Дата | Что сделано |
|---|---|
| 15.05.2026 | Добавлен раздел «Лента сообщений» в дашборд Линнимакс — фильтрация по тональности (позитив / нейтрал / негатив), данные из ручного экспорта BA за апрель 2026. Создан `tonal/parse.js` для конвертации экспорта в `messages-YYYY-MM.json`. Дашборд опубликован на GitHub Pages. |
| 15.05.2026 | Добавлена тональность за март 2026 методом ручной классификации (Claude Sonnet 4.6). Методология скорректирована под Brand Analytics: промо-контент и описания продуктов с позитивной подачей → позитив. Результат: 83+ / 88○ / 1− (совпадение с BA 100%). Лента сообщений теперь доступна для марта и апреля 2026. |
...
| 15.05.2026 | Подключён BA Feed API (`api.br-analytics.ru`, Theme ID `14159496`). Создан `fetch_messages.js` — ежедневная автовыгрузка сообщений + классификация тональности через Claude Haiku via OpenRouter. Добавлен шаг в GitHub Actions. Секреты: `BA_LOGIN`, `BA_SECRET_KEY`, `OPENROUTER_API_KEY`. Вручную классифицированы сообщения за май 2026: 8+ / 3○ / 0−. |
| 16.05.2026 | В `fetch_data.js` добавлена защита от перезаписи нулями: если BA Statistical API вернул `totalMsgs === 0` (возможен при rate limit при повторном запуске за день) — файл не перезаписывается, выводится предупреждение в лог. |
